...
export const Main = styled.div`
  ...
  width: 100%;
  padding: ${display.mobile.xxl} 0;
  padding: 0;
  @media screen and (min-width: ${breakpoints.md}) {
    padding: ${display.desktop.xl} 0;
  }
  @media screen and (max-width: ${breakpoints.xl}) {
    padding: calc(${display.desktop.xl} * 1.125) 0;
...
export const Column = styled(Col)`
  margin: 0;
  ${({ paddingsmall }) =>
    paddingsmall ? 'padding: 0.15rem 0.5rem;' : 'padding: 1rem 1rem'};
...
export const StyledRow = styled(Row)`
  ...
  & > ${Column} {
    ...
    ${({ flex }) =>
      flex === 'true' ? 'height: -webkit-fill-available;' : null};
